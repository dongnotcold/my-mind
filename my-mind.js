...
MM.Command.InsertChild = Object.create(MM.Command, {
    ...
    keys: { value: [{ keyCode: 9, ctrlKey: false }, { keyCode: 45 }] },
});
...
MM.Command.Edit = Object.create(MM.Command, {
    ...
    keys: { value: [{ keyCode: 32 }, { keyCode: 113 }] },
});
...
MM.Format.Mup._MupToMM = function (item) {
    ...
    if (item.ideas) {
        ...
        json.children = data.map(function (item) {
            return item.child;
        });
    }
...
MM.Backend.WebDAV._request = function (method, url, data) {
    ...
    Promise.send(xhr, data).then(
        ...
        function (xhr) {
            promise.reject(new Error("HTTP/" + xhr.status + "\n\n" + xhr.responseText));
        }
    );
        ...
        function (response) {
            if (response && response.downloadUrl) {
                ...
                Promise.send(xhr).then(
                    function (xhr) {
                        promise.fulfill({ data: xhr.responseText, name: response.title, mime: response.mimeType });
                    },
                    function (xhr) {
